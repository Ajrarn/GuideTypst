...
#example(label: <exBoite>)[Exemple de Code][
  ```typst
...
Cette boîte(cf. @exBoite) permet de visualiser le code en haut et son exécution en bas. Ce premier exemple ne contient que du texte#footnote[Une citation de Dune de Frank Herbert.] sans mise en page, donc rien d'extraordinaire pour l'instant.
...
Parfois, je ne pourrais pas afficher à la fois le code et la visualisation de ses effets. Dans ce cas, j'afficherai simplement le bloc de code.

== Astuce

#tips(label:<tipsLang>)[Première astuce][
  ```typst
  #set text(lang:"fr")
  ```
...
Cette boite(cf. @tipsLang) permet de mettre en valeurs des astuces, et celle qui y figure est vraiment pratique.
...
- La première partie permet de découvrir les premiers éléments de langage indispensables. En particulier la syntaxe du mode _Markup_(cf. @modesSyntaxiques), elle abordera également un peu de code, le minimum à connaître pour personnaliser son projet et utiliser des packages fournis par la communauté.
