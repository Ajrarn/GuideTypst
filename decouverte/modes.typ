...
#example[Formule au sein d'une ligne][
  ```typst
...
On peut également souhaiter que la formule soit isolée dans sa propre ligne, auquel cas on va utiliser des espaces en plus des \$ #footnote[un espace après le premier \$, et un espace avant le dernier \$.].

#example[Formule dans un paragraphe][
  ```typst
...
Tant que l'instruction n'est pas terminée, on reste en mode _Code_ et il n'est donc pas nécessaire d'utiliser le caractère \# pour tous les appels de fonction.
...
Et oui, on doit pouvoir également afficher des \$ ou des \# ou tous les autres caractères qui ont une signification :

#example[Caractères spéciaux][
  ```typst
